...
builder.Services.AddSingleton<ProjectNameResolver>();
...
app.MapPost("/api/v1/questions", async (QuestionSubmission x, HttpRequest request, PendingQuestionStore s, IHookTokenProvider tokens, AlertEngine alerts, INotificationOutbox outbox, ProjectNameResolver projectNames, CancellationToken ct) => { if (!request.Headers.Authorization.ToString().Equals("Bearer " + tokens.GetToken(x.RuntimeId), StringComparison.Ordinal)) return Results.Unauthorized(); var q = s.Create(x.RuntimeId, x.SessionId, x.QuestionsJson); var project = await projectNames.ResolveAsync(x.Cwd, x.RuntimeId, ct); var c = alerts.QuestionPending(q, project); if (c is not null) outbox.Enqueue(c, alerts.GetCooldownSeconds(c.NotificationType)); return Results.Created($"/api/v1/questions/{q.Id}", q); });
...
static async Task RunLifecycleCommandAsync(string command, string[] args)
{
    ...
    if (command == "install")
    {
        ...
        await wsl.InstallAsync(exe, wslDistro, dry); return;
    }
...
static async Task RunFirstRunIfNeededAsync(LocalAppDataLayout layout)
{
    ...
    try
    {
        var runner = new ProcessRunner(); var discovery = new RuntimeDiscovery(runner); var exe = Process.GetCurrentProcess().MainModule?.FileName ?? "AgentSupervisor.App.exe";
        var wslDistro = Environment.GetEnvironmentVariable("AGENTSUPERVISOR_WSL_DISTRO") ?? "Ubuntu";
...
public sealed record QuestionSubmission(string RuntimeId, string? SessionId, string QuestionsJson, string? Cwd = null);
